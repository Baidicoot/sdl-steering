...
def train_models(train_cfg: dotdict, dataset_cfg: dict, files_cfg: dotdict, log_cfg: dotdict):
    ...
    model = sae.SparseLinearAutoencoder(activation_size, latent_dim, 0.0001, device=train_cfg.device)
    ...
    for epoch in range(train_cfg.n_epochs):
        ...
        for chunk in chunk_idxs:
            ...
            for batch in tqdm.tqdm(dataloader):
                ...
                #loss, mse, outputs, new_optimizer_state = train_step(model_meta, params, buffers, optimizer_state, optimizer, batch, return_hidden=True, return_output=True)
                #optimizer_state = new_optimizer_state
                #loss, mse, hiddens, preds = ensemble.step_batch(batch)

                optimizer.zero_grad()
                loss, mse, hiddens, preds = model(batch)
                loss.backward()
                optimizer.step()
                ...
                if i % 100 == 0:
                    if prev_encoder is None:
                        prev_encoder = model.encoder.detach().clone()
                        cosine_sim_change = torch.tensor(1.0)
                    else:
                        cosine_sim_change = torch.cosine_similarity(prev_encoder, model.encoder.detach(), dim=0).mean()
                        prev_encoder = model.encoder.detach().clone()
                    
                    wandb.log({
                        "cosine_sim_change": cosine_sim_change.item(),
                    }, commit=False)
                ...
                if files_cfg.wandb_config is not None:
                    ...
                    wandb.log({
                        ...
                        "bias_norm": {l1: torch.norm(model.encoder_bias).item() for i, l1 in enumerate(l1_range)},
                    }, commit=True)
    ...
    model_dict = {model.l1_penalty: model for model in models}
